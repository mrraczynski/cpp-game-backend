...
namespace http_handler {
    ...
    bool RequestHandler::IsMapsRequest(const std::vector<std::string_view>& target_vec)
    {
    ...
    bool RequestHandler::IsGameRequest(const std::vector<std::string_view>& target_vec)
    {
    ...
    bool RequestHandler::IsGameJoinRequest(const std::vector<std::string_view>& target_vec)
    {
        return IsGameRequest(target_vec) &&
            target_vec[4].compare("join") == 0;
    }

    bool RequestHandler::IsGamePlayersRequest(const std::vector<std::string_view>& target_vec)
    {
        return IsGameRequest(target_vec) &&
