...
//#if 0
            ...
            AND_THEN("it can`t select previous channel")
            {
                CHECK_THROWS_AS(tv.SelectLastViewedChannel(), std::logic_error);
                tv.TurnOn();
                CHECK_NOTHROW(tv.SelectChannel(10));
                CHECK_NOTHROW(tv.SelectChannel(15));
                CHECK_NOTHROW(tv.SelectLastViewedChannel());
...
#pragma region SelectChannelTests_TVIsOff

            AND_WHEN("SelectChannel command is entered when TV is turned off") {
...
#pragma endregion SelectChannelTests_TVIsOff

#pragma region SelectPreviousChannel_TVIsOff

            AND_WHEN("SelectPreviousChannel command is entered when TV is turned off") {
...
#pragma endregion SelectPreviousChannel_TVIsOff
...
#pragma region SelectChannelTests_TVIsOn

            AND_WHEN("SelectChannel command is entered with no params") {
                run_menu_command("SelectChannel"s);

                THEN("invalid channel is printed") {
                    expect_output("Invalid channel\n"s);
                }
            }

            AND_WHEN("SelectChannel command is entered with incorrect parrams") {
                run_menu_command("SelectChannel some param"s);
            ...
            AND_WHEN("SelectChannel command is entered with wrong channel namber (more then maximal)") {
                run_menu_command("SelectChannel "s + std::to_string(TV::MAX_CHANNEL + 1));

                THEN("channel out of range is printed") {
                    expect_output("Channel is out of range\n"s);
                }
            }

            AND_WHEN("SelectChannel command is entered with wrong channel namber (less then minimal)") {
                run_menu_command("SelectChannel "s + std::to_string(TV::MIN_CHANNEL - 1));
            ...
            AND_WHEN("SelectChannel command is entered with correct params") {
                run_menu_command("SelectChannel 8"s);

                THEN("tv is set to correct channel") {
                    CHECK(tv.GetChannel() == 8);
                    expect_empty_output();
                }
            }

#pragma endregion SelectChannelTests_TVIsOn

#pragma region SelectPreviousChannel_TVIsOn

            AND_WHEN("SelectPreviousChannel command is entered when TV is turned on") {
                run_menu_command("SelectChannel 8"s);
                run_menu_command("SelectChannel 12"s);
                run_menu_command("SelectPreviousChannel"s);

                THEN("tv is set to previous channel") {
                    CHECK(tv.GetChannel() == 8);
                    expect_empty_output();
                }
            }

            AND_WHEN("SelectPreviousChannel command is entered second time") {
                run_menu_command("SelectChannel 8"s);
                run_menu_command("SelectChannel 12"s);
                run_menu_command("SelectChannel 14"s);
                run_menu_command("SelectPreviousChannel"s);
                run_menu_command("SelectPreviousChannel"s);
            ...
            AND_WHEN("SelectPreviousChannel command is entered third time") {
                run_menu_command("SelectChannel 8"s);
                run_menu_command("SelectChannel 12"s);
                run_menu_command("SelectChannel 14"s);
                run_menu_command("SelectPreviousChannel"s);
...
#pragma endregion SelectPreviousChannel_TVIsOn
